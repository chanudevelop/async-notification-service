...
## 10. 읽음 처리 동시성 — DB 조건부 UPDATE

### 결정
- `PATCH /notifications/{id}/read` 엔드포인트 + `X-User-Id` 헤더 (본인 알림만)
- Repository에서 `UPDATE notifications SET read_at = NOW() WHERE id = ? AND read_at IS NULL` 조건부 UPDATE
- 응답에 `firstRead` 반환 — 첫 호출은 true, 이후 호출은 false

### 왜 자바 메모리 검사가 아닌 DB 조건부 UPDATE인가

자바 도메인 메서드의 `if (readAt == null)` 검사는 두 스레드가 동시에 진입하면 둘 다 통과해 readAt이 last-writer-wins로 덮입니다. 비즈니스 영향은 거의 없지만 첫 읽음 시점이 보존되지 않는 정확성 문제가 남습니다.

조건부 UPDATE는 SQL 안에 `WHERE read_at IS NULL`을 두어 DB가 차단을 책임집니다. 100개 스레드가 동시에 호출해도 row-level lock으로 직렬화되어 첫 통과만 1행 갱신하고 이후는 0행 갱신으로 처리됩니다. affected rows로 첫 호출 여부를 식별할 수 있어 응답에 `firstRead`를 정확히 줄 수 있습니다.

이건 알림 등록 멱등성에서 DB UNIQUE 제약 + Optimistic INSERT를 쓴 패턴과 같은 철학입니다. 자바 메모리 검사가 아닌 **DB가 단일 진실 원천**.

### 권한 검증을 Service에서 명시 분리한 이유

UPDATE 안에 `recipient_id` 조건을 같이 넣으면 SQL 1번으로 끝나지만, 0행 갱신 케이스가 "권한 위반 / 알림 없음 / 이미 읽음" 셋이 구분 안 됩니다. HTTP 상태 코드가 404/403/200으로 명확히 나뉘는 게 클라이언트 입장에서 가치 있어 Service에서 `findById` → recipientId 비교 → UPDATE 호출 흐름으로 분리했습니다. SQL 2번이지만 ms 단위 차이는 무시 가능.

### 검증

`ConcurrencyIntegrationTest`에서 100개 스레드가 같은 알림에 동시에 markAsRead 호출 → `firstRead=true` 정확히 1건, 나머지 99건 `false`인 걸 자동 증명.
...
| 읽음 처리 | status와 분리된 readAt 컬럼 + DB 조건부 UPDATE로 동시성 차단 |
